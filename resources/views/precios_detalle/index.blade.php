        ...
        <div class="card-body pt-0">
            <div class="table-responsive scrollbar">
                <table id="table_precio_productos" class="table display">
                    ...
                    <tbody>
                        @foreach ($precioDetalles as $precioDetalle)
                            <tr>
                                <td>{{ $precioDetalle->producto->nombre }}</td>
                                <td>{{ $precioDetalle->precio->precio_Monto }}</td>
                                <td class="text-end">
                                    <form action="{{ route('precios_detalle.destroy', $precioDetalle->id) }}"
                                        method="POST">
                                        <a href="{{ route('precios_detalle.edit', $precioDetalle->id) }}">
